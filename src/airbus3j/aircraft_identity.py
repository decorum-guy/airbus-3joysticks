...
def _candidate_package_dirs(roots: Iterable[Path]) -> list[Path]:
    ...
    for root in roots:
        ...
        for child in children:
            if any(hint in child.name.lower() for hint in PACKAGE_HINTS):
    ...
    for package_dir in _candidate_package_dirs(roots):
        ...
        for cfg_path in cfg_paths:
            ...
            for block in blocks:
                ...
                if wanted and not exact and not a320ish:
                    continue
...
def identify_from_disk(title: str | None, env: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    result["classification"] = best.get("classification") if best else classify_aircraft(title)
    return result
...
def main() -> None:
    ...
    if os.name == "nt":
        try:
            ...
            report["simconnect"] = {"connected": False, "error": f"{type(exc).__name__}: {exc}"}
        finally:
            if sc is not None:
                try:
                    sc.Close()
